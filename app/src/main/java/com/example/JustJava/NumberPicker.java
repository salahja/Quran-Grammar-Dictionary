...
public class NumberPicker extends LinearLayout {
   ...
   */
  private final EditText mSurahPicker;
  ...
  public NumberPicker(Context context, AttributeSet attrs, int defStyle) {
    ...
    // input text
    mSurahPicker = findViewById(R.id.surahpicker);
    mSurahPicker.setEnabled(false);
    mSurahPicker.setFocusable(false);
    mSurahPicker.setImeOptions(EditorInfo.IME_ACTION_NONE);
  ...
  protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
    ...
    final int inptTxtMsrdWdth = mSurahPicker.getMeasuredWidth();
    final int inptTxtMsrdHght = mSurahPicker.getMeasuredHeight();
    final int inptTxtLeft = (msrdWdth - inptTxtMsrdWdth) / 2;
    final int inptTxtTop = (msrdHght - inptTxtMsrdHght) / 2;
    final int inptTxtRight = inptTxtLeft + inptTxtMsrdWdth;
    final int inptTxtBottom = inptTxtTop + inptTxtMsrdHght;
    mSurahPicker.layout(inptTxtLeft, inptTxtTop, inptTxtRight, inptTxtBottom);
    mSelectedTextCenterX = mSurahPicker.getX() + mSurahPicker.getMeasuredWidth() / 2f - 2f;
    mSelectedTextCenterY = mSurahPicker.getY() + mSurahPicker.getMeasuredHeight() / 2f - 5f;
  ...
  public void setEnabled(boolean enabled) {
    ...
    mSurahPicker.setEnabled(enabled);
  }
  ...
  private void tryComputeMaxWidth() {
    ...
    maxTextWidth += mSurahPicker.getPaddingLeft() + mSurahPicker.getPaddingRight();
...
//        }
  ...
  public void setDisplayedValues(String[] displayedValues) {
    ...
    if (mDisplayedValues != null) {
      ...
      mSurahPicker.setRawInputType(InputType.TYPE_TEXT_FLAG_MULTI_LINE
            | InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS);
    } else {
      mSurahPicker.setRawInputType(InputType.TYPE_CLASS_NUMBER);
    }
    updateInputTextView();
    initializeSelectorWheelIndices();
    tryComputeMaxWidth();
  ...
  protected void onDraw(Canvas canvas) {
    ...
    if (isHorizontalMode()) {
      ...
      y = mSurahPicker.getBaseline() + mSurahPicker.getTop();
    ...
    for (int i = 0; i < selectorIndices.length; i++) {
      ...
      if ((showSelectorWheel && i != mWheelMiddleItemIndex)
            || (i == mWheelMiddleItemIndex && mSurahPicker.getVisibility() != VISIBLE)) {
        float textY = y;
        if (!isHorizontalMode()) {
  ...
  private void updateInputTextView() {
    ...
    CharSequence beforeText = mSurahPicker.getText();
    if (text.equals(beforeText.toString())) {
      return;
    }

    mSurahPicker.setText(text);
  }
  ...
  private void postSetSelectionCommand(int selectionStart, int selectionEnd) {
    if (mSetSelectionCommand == null) {
      mSetSelectionCommand = new SetSelectionCommand(mSurahPicker);
    } else {
  ...
  public void setSelectedTextColor(@ColorInt int color) {
    ...
    mSurahPicker.setTextColor(mSelectedTextColor);
  }
  ...
  public void setSelectedTextSize(float textSize) {
    ...
    mSurahPicker.setTextSize(pxToSp(mSelectedTextSize));
  }
  ...
  public void setTypeface(Typeface typeface) {
    mTypeface = typeface;
    if (mTypeface != null) {
      mSurahPicker.setTypeface(mTypeface);
      setSelectedTypeface(mSelectedTypeface);
    } else {
      mSurahPicker.setTypeface(Typeface.MONOSPACE);
    }
  }
